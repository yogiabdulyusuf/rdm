...
class rdm_trans(models.Model):
        ...
        for trans in self:
            ...
            for rdm_config in config:
                if rdm_config.trans_delete_allowed == True:
                    ...
                    self.env['rdm.customer.coupon'].write(customer_coupon_ids, {'state':'req_delete'})
                    for customer_coupon_id in customer_coupon_ids:
                        ...
                        if customer_coupon_detail_ids:
                            self.env['rdm.customer.coupon.detail'].write(customer_coupon_detail_ids, {'state':'req_delete'})

                    customer_point_ids = self.env['rdm.customer.point'].search([('trans_id','=', trans.trans_id)])
                    self.env['rdm.customer.point'].write(customer_point_ids, {'state':'req_delete'})
                    for customer_point_id in customer_point_ids:
                        ...
                        if customer_point_detail_ids:
                            self.env['rdm.customer.point.detail'].write(customer_point_detail_ids, {'state':'req_delete'})
        ...
        for trans in self:
            ...
            for rdm_config in config:
                ...
                if approver.user_id.id == trans.uid:
                    trans_detail_ids = trans.trans_detail_ids
                    for trans_detail in trans_detail_ids:
                        self.env['rdm.trans.detail'].write(trans_detail.id, {'state':'delete'})

                    customer_coupon_ids = self.env['rdm.customer.coupon'].search([('trans_id','=', trans.trans_id)])
                    self.env['rdm.customer.coupon'].write(customer_coupon_ids, {'state':'delete'})
                    for customer_coupon_id in customer_coupon_ids:
                        customer_coupon_detail_ids = self.env['rdm.customer.coupon.detail'].search([('customer_coupon_id','=',customer_coupon_id)])
                        if customer_coupon_detail_ids:
                    ...
                    customer_point_ids = self.env['rdm.customer.point'].search([('trans_id','=', trans.trans_id)])
                    self.env['rdm.customer.point'].write(customer_point_ids, {'state':'delete'})
                    for customer_point_id in customer_point_ids:
                        customer_point_detail_ids = self.env['rdm.customer.point.detail'].search([('customer_point_id','=',customer_point_id)])
                        if customer_point_detail_ids:
                            self.env['rdm.customer.point.detail'].write(customer_point_detail_ids, {'state':'delete'})
        ...
        for trans in self:
            ...
            for rdm_config in config:
                ...
                if approver.user_id.id == trans.uid:
                    ...
                    trans_detail_ids = trans.trans_detail_ids
                    for trans_detail in trans_detail_ids:
                        self.env['rdm.trans.detail'].write(trans_detail.id, {'state':'done'})

                    customer_coupon_ids = self.env['rdm.customer.coupon'].search([('trans_id','=', trans.trans_id)])
                    self.env['rdm.customer.coupon'].write(customer_coupon_ids, {'state':'active'})

                    customer_point_ids = self.env['rdm.customer.point'].search([('trans_id','=', trans.trans_id)])
                    self.env['rdm.customer.point'].write(customer_point_ids, {'state':'active'})
                else:
    ...
    #     return self.env['rdm.trans.schemas'].browse(self.trans_id);
    #
    ...
    def _get_customer_filters(self, trans_schemas_id):
        ...
        for trans in self:
            ...
            args = [('trans_id','=', trans_schemas_id)]
            res = self.env['rdm.trans.schemas'].search(args)
    ...
    @api.one
    def _get_valid_total(self, trans_schemas_id):
        ...
        for trans in self:
            ...
            for trans_schemas_id in trans_schemas_ids:
                ...
                for trans_detail in trans.trans_detail_ids:
                    ...
                    status, message = self._get_tenant_filters(schemas_id, tenant_id)
                    _logger.info(message)
                    if status:
                        valid_amount = valid_amount + trans_detail.total_amount

                args = [('trans_id','=', trans_schemas_id.id)]
                res = self.env['rdm.trans.schemas'].search(args)
                ...
                res.write(trans_schemas_data)
                _logger.info('End Get Valid Total Filter')


    def _calculate_add_coupon_and_point(self, trans_id):
        ...
        for trans in self:
            ...
            for trans_schemas_id in trans_schemas_ids:
                ...
                for trans_detail_id in trans_detail_ids:
                    ...
                    schemas_status, message = self._get_tenant_filters(schemas_id, tenant)
    ...
    def _calculate_global_add_coupon_and_point(self, trans_id):
    ...
    @api.one
    def _calculate_trans_priority_per_schemas(self, trans_id):
        ...
        for trans in self:
            ...
            for trans_schemas_id in trans_schemas_ids:
                ...
                for num in range(0,number_of_detail):
                    ...
                    args = [('trans_detail_id', '=', trans_detail_coupon.id)]
                    res = self.env['rdm.trans.detail.coupon'].search(args)

                    trans_data = {}
                    trans_data.update({'priority':priority})
                    trans_data.update({'state':'done'})
                    res.write(trans_data)
                ...
                for num in range(0,number_of_detail):
                    ...
                    args = [('trans_detail_id','=', trans_detail_point_id)]
                    res = self.env['rdm.trans.detail.point'].search(args)
                    trans_data = {}
                    trans_data.update({'priority':priority})
                    trans_data.update({'state':'done'})
                    res.write(trans_data)
    ...
    @api.one
    def _calculate_valid_coupon_and_point(self, trans_id):
                    ...
                    if trans_detail_id:
                        ...
                        if diff_spend_amount <= 0:
                            ...
                            if diff_spend_amount >= trans_detail_id.total_amount:
                                ...
                                args = [('trans_detail_id', '=', trans_detail_point.id)]
                                res = self.env['rdm.trans.detail.point'].search(args)
                                ...
                                trans_data = {}
                                trans_data.update({'valid_point': valid_point})
                                res.self.env['rdm.trans.detail.point'].write(trans_data)
                    else:
                        valid_point = trans_detail_point.point
                        ...
                        trans_data = {}
                        trans_data.update({'valid_point': valid_point})
                        self.env['rdm.trans.detail.point'].write(trans_data)

        _logger.info('End Calculate Valid Coupon and Point')

    @api.one
    def _close_trans_detail(self, trans_id):
        ...
        for trans in self:
            ...
            for trans_detail_id in trans_detail_ids:
                ...
                args = [('trans_id','=', trans_detail_id.id), ]
                res = self.env['rdm.trans.detail'].search(args)
                trans_data = {}
                trans_data.update({'state':'done'})
                res.write(trans_data)
    ...
    @api.one
    def _calculate_schemas_total_coupon_and_point(self, trans_id):
        ...
        for trans in self:
            ...
            for trans_schemas_id in trans_schemas_ids:
                ...
                trans_coupon_ids = self.env['rdm.trans.detail.coupon'].search(args)
                trans_coupons = self.env['rdm.trans.detail.coupon'].browse(trans_coupon_ids)
                ...
                trans_point_ids = self.env['rdm.trans.detail.point'].search(args)
                trans_points = self.env['rdm.trans.detail.point'].browse(trans_point_ids)
                ...
                args_schemas = [('trans_id','=', trans_schemas_id.id)]
                res = self.env['rdm.trans.schemas'].search(args_schemas)
                trans_schemas_data = {}
                ...
                if customer_id.email_required and customer_id.receive_email:
                    ...
                    if schemas_id.limit_point_per_periode > -1:
                        periode_point = self.periode_total_point(schemas_id, customer_id)
    ...
    @api.one
    def _calculate_total_coupon_and_point(self, trans_id):
    ...
    def _define_trans_schemas(self):
        ...
        for trans in self:
            ...
            for schemas in active_schemas:
                trans_schemas_data = {}
                trans_schemas_data.update({'trans_id': trans.id})
                trans_schemas_data.update({'schemas_id': schemas.id})
                trans_schemas_id = self.env['rdm.trans.schemas'].create(trans_schemas_data)
    ...
    def _post_calculation(self):
        ...
        self._calculate_add_coupon_and_point(trans_id)
        #Calculate Global Additional Coupon and Point for All Transaction Detail
        self._calculate_global_add_coupon_and_point(trans_id)
        #Calculate Priority for Coupon and Point
        self._calculate_trans_priority_per_schemas(trans_id)
        #Calculate Valid Coupon and Point
        self._calculate_valid_coupon_and_point(trans_id)
        #Close Trans Detail
        self._close_trans_detail(trans_id)
        #Calculate Schemas Total Coupon and Point
        self._calculate_schemas_total_coupon_and_point(trans_id)
        #Calculate Total Coupon and Point for All Schemas
        self._calculate_total_coupon_and_point(trans_id)
        #Generate Coupon
        self._generate_coupon(trans_id)
        #Generate Point
        self._generate_point(trans_id)
